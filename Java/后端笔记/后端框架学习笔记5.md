...
![redis&mysql](../../img/redis&mysql.jpg)
...
![gundongfabu](../../img/gundongfabu.png)



##### 139 : 灰度发布和灰度发布A/B测试
A : **灰度发布也叫金丝雀发布**，起源是，矿井工人发现，金丝雀对瓦斯气体很敏感，矿工会在下井之前，先放一只金丝雀到井中，如果金丝雀不叫了，就代表瓦斯浓度高。
![huidufabu](../../img/huidufabu.png)**在灰度发布开始后，先启动一个新版本应用，但是并不直接将流量切过来，而是测试人员对新版本进行线上测试，启动的这个新版本应用，就是我们的金丝雀**。如果没有问题，那么可以将少量的用户流量导入到新版本上，然后再对新版本做运行状态观察，收集各种运行时数据，**如果此时对新旧版本做各种数据对比，就是所谓的A/B测试**。
...
A : 步骤如下, 一开始的状态如下
![chuangjianmergerequest](../../img/chuangjianmergerequest.png)
1.创建新的 Merge Request, 选择合并的分支
![mergebranch](../../img/mergebranch.jpg)
2.填写Merge Request相关信息并提交
![mergeinfo](../../img/mergeinfo.jpg)
3.收到Merge Request, 并处理
![shoudaomerge](../../img/shoudaomerge.jpg)



##### 141: Git Merge处理和Code Review
A : 当收到了Merge的请求之后，会出现如图的提示，然后 **审核人点击Merge** ，merge完成之后会有记录。
![codereviewmerge](../../img/codereviewmerge.png)
![gitmerge](../../img/gitmerge.png)
![mergeresult](../../img/mergeresult.png)
...
A : 大致如下三个是最常用的功能
1.设置
![postmansetting](../../img/postmansetting.jpg)
1.去除代理和SSL认证
![postmanproxy](../../img/postmanproxy.png)
![postmanssl](../../img/postmanssl.png)
2.设置环境变量
![postmanenv](../../img/postmanenv.jpg)
设置了环境变量之后,通过{{关键字}}的方式来使用,如{{USER}},{{PWD}}等.
3.去除Cookie
![postmancookie](../../img/postmancookie.png)
...
![ideahotswap](../../img/ideahotswap.jpg)
设置好了之后, 在Debug的时候, 我们选择recompile即可。
![ideahotswap2](../../img/ideahostswap2.jpg)
2.使用了jreble的情况下的调试,操作如下即可
![jrebledebug](../../img/jrebledebug.jpg)
...
![codestructure1](../../img/codestructure1.jpg)
...
![codestructure2](../../img/codestructure2.jpg)
...
```
程序控制流图如下：
![codestructure3](../../img/codestructure3.jpg)
...
![ideaimport1](../../img/ideaimport1.jpg)
![ideaimport2](../../img/ideaimport2.jpg)
