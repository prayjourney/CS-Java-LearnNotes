...
```
:six:. 在Manjaro Linux之中没有`service`命令, 所以无法使用`service mysql start`启动mysql服务, 但是提供的`systemctl`命令和`service`是等效的, 我们可以使用`systemctl start mysql`来启动mysql服务.
